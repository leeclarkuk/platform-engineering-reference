...
`make check-prohibited`, `make check-m0-assertions`,
`make check-no-cloud-mutation`, `make platform-test`,
`make terraform-validate`, `make gitops-validate`,
`make observability-validate`, `git`, `grep`).
