...
| M1 Platform contract and CLI | Versioned contract schema; `platform` CLI `doctor`/`validate`/`create` without cloud credentials | This PR (`api/`, `cmd/platform`, `templates/` Helm skeleton, `testdata/`, `go.mod`) | Close this row in the PR; no AWS, no GitOps apply, no journeys; do not copy `developer-platform/` from archive |
| M2 AWS foundations (local validate) | Terraform for bootstrap/network/workload with **no** Kubernetes objects; `terraform validate` without credentials | Absent (`infra/` forbidden; `aws-foundations-builder` is dormant) | New AWS modules on this lineage; do not restore archive `terraform/` or `landing-zones/` |
